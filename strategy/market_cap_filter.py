...
# Try to import settings, but use env vars as fallback
try:
    from config.settings import settings
    HAS_CONFIG = True
except ImportError:
    HAS_CONFIG = False
    logger.warning("config.settings not found, using environment variables")
    ...
    def __init__(self, max_market_cap_usd: float = 60_000_000):
        ...
        if HAS_CONFIG:
            try:
                ...
                logger.warning("COINMARKETCAP_API_KEY not found in settings, checking env")
                self.cmc_api_key = os.getenv("COINMARKETCAP_API_KEY", "YOUR_CMC_API_KEY_HERE")
        else:
            # Use environment variable directly
            self.cmc_api_key = os.getenv("COINMARKETCAP_API_KEY", "YOUR_CMC_API_KEY_HERE")
            if self.cmc_api_key == "YOUR_CMC_API_KEY_HERE":
                logger.warning("COINMARKETCAP_API_KEY not set in environment")
